...
- 如果encrypt-signature.enable = true ，则mpc-node会对签名结果进行加密，否则返回原始签名结果。
...
- encrypt-signature.public-key-path 和callback.server.public-key-path 都使用的Secp256r1算法。为了简单化，建议使用同一把公私钥。