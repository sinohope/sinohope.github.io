...
- 对于独立部署mpc-node的用户户，在调用接口`/v1/waas/mpc/web3/sign_message`和`/v1/waas/mpc/wallet/advance/sign_raw_data`进行签名时， mpc-node可对签名结果进行加密保护。
- 加密保护使用Secp256r1算法，公钥加密，私钥解密。
- 影响客户调用接口/v1/waas/mpc/web3/sign_result`查询签名结果时，需要解密后使用。
- 用户客户为mpc-node配置了Callback 回调服务， 回调服务接收到的签名结果也需要解密后再使用。