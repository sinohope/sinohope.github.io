...
- If `storage.t` in the configuration is greater than 1, init and other operation commands only need to be executed t times. The t-n password needs to be generated using a special tool and cannot be entered for No arbitrary input is allowed.
