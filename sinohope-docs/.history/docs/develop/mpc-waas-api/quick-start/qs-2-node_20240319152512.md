...
- Set a password for the MPC node (including uppercase letters, lowercase letters, numbers, special characters, with a length of more than 12 digits). This password is used to protect your private key sharding and other private data. Please keep it properly, do not leak it, and do a Safety Redundancy backup . The node will be required to enter the password when it starts up.
