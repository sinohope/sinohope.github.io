...
- The password of sinohope-mpc-node adopts the combination of t-n, t must be less than or equal to n (such as 2-3, only t password needs to be entered during init and unseal).
- If `storage.t` in the configuration is set to 1, the operation command only needs to be executed once, and the password can be generated manually.
- If `storage.t` in the configuration is greater than 1, the operation command needs to be executed t times, and the t-n password needs to be generated using a special tool and cannot be entered freely.
...
For users who deploy mpc-node independently, when calling the interfaces `/v1/waas/mpc/web3/sign_message` and `/v1/waas/mpc/wallet/advance/sign_raw_data` to sign, mpc-node can sign the result Encryption protection.

- Encryption protection uses the Secp256r1 algorithm, public key encryption, and private key decryption.
- When customers call the interface `/v1/waas/mpc/web3/sign_result` to query the signature result, they need to decrypt it before use.
- If mpc-node is configured with a Callback callback service, the signature result received by the callback service also needs to be decrypted before use.


### 5.1 Generate encrypted public and private keys
...
Enable the signature result encryption function in the configuration file config.toml and specify the encrypted public key.
```
[encrypt-signature]
enable=true
...
- If `encrypt-signature.enable = true`, mpc-node will encrypt the signature result, otherwise return the original signature result.
- `encrypt-signature.public-key-path` The public key of the encrypted signature result, using pem format file.
- `callback.server.public-key-path` mpc-node and Callback service communication data signature verification are common. The Callback service uses the private key to give a signature to the communication data. mpc-node uses this public key to perform data verification. Use pem format file.
- Secp256r1 algorithm used by both `encrypt-signature.public-key-path` and `callback.server.public-key-path`. For simplicity, it is recommended to use the same public and private key.


### 5.3 Decrypt the signature result


In the callback service demo, the callback result (encrypted signature data) is decrypted. Please refer to the callback service example provided by Sinohope: [GitHub - sinohope/mpc-node-callback-demo](https://github.com/sinohope/mpc-node-callback-demo)
